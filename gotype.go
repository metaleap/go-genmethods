		...
		if somedecl, _ := topleveldecl.(*ast.GenDecl); somedecl != nil {
			...
			for _, spec := range somedecl.Specs {
				if tdecl, _ := spec.(*ast.TypeSpec); tdecl != nil && tdecl.Name != nil && tdecl.Name.Name != "" && tdecl.Type != nil {
					...
					t.G.TVal, t.Underlying.AstExpr = udevgogen.TrNamed("", t.Name), goAstTypeExprSansParens(tdecl.Type)
					t.G.TPtr, t.G.TSl = udevgogen.TrPtr(t.G.TVal), udevgogen.TrSlice(t.G.TVal)
					t.G.ThisVal, t.G.ThisPtr = udevgogen.V.This.T(t.G.TVal), udevgogen.V.This.T(udevgogen.TrPtr(t.G.TVal))
					this.Types.Add(t)
