...
func (this *GentIndexMethods) genIndicesOfMethod(t *gent.Type, methodName string, resultsCapFactor uint, predicate bool) *SynFunc {
	...
	foreachitemcheckcond := GEN_IF(predicate, Then(
		ˇ.Ok.Of(This.At(ˇ.I)), // ok(this[i])
	), Else(
		This.At(ˇ.I).Eq(ˇ.V), // this[i] == v
	))
	...
	return t.G.This.Method(methodName, arg).Rets(ret).
		...
		Code(
			GEN_IF(resultsCapFactor > 0,
				ˇ.R.Set(B.Make.Of(ret.Type, L(0), B.Len.Of(This).Div(L(resultsCapFactor)))), // r = make([]int, 0, len(this) / ‹resultsCapFactor›)
			),
			ForEach(ˇ.I, None, This, // for i := range this
...
func (this *GentIndexMethods) genIndexOfMethod(t *gent.Type, methodName string, isLast bool, variadic bool, predicate bool) *SynFunc {
	arg, forloop := this.indexMethodArg(t, variadic, predicate), ForEach(ˇ.I, None, This) // for i := range this
	if isLast {
		forloop = For(ˇ.I.Let(B.Len.Of(This).Minus(L(1))), (ˇ.I.Gt(-1)), (ˇ.I.Decr1())) // for i := len(this)-1; i > -1; i--
	}
	...
	return t.G.This.Method(methodName, arg).Rets(ˇ.R.OfType(T.Int)).
		...
		Code(
			forloop.Code(GEN_BYCASE(USUALLY(
				If(This.At(ˇ.I).Eq(ˇ.V), Then( // if this[i] == v
					ˇ.R.Set(ˇ.I), // r = i
					K.Return)),
			), UNLESS{
				predicate: If(ˇ.Ok.Of(This.At(ˇ.I)), Then( // if ok(this[i])
					ˇ.R.Set(ˇ.I), // r = i
					K.Return)),
				variadic: ForEach(ˇ.J, None, arg, // for j := range v
					If(This.At(ˇ.I).Eq(ˇ.V.At(ˇ.J)), Then( // if this[i] == v[j]
						ˇ.R.Set(ˇ.I), // r = i
						K.Return))),
			})),
			ˇ.R.Set(-1), // r = -1
